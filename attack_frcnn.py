...
# for patch
parser.add_argument("--obj_path", default="objs/simple_door_key_scaled.obj", type=str, #초기 3D 적대적 객체 obj 파일의 위치 지정
...
def attack(images, bboxes, labels, net, trainer): #(이미지들,bbox정보, 정답 클래스 라벨,모델,학습을 시키는 trainer 객체)
    ...
    #홈 부분의 정점들 로드
    groove_coords = load_groove_coords("groove_points_scaled.txt")  # (M, 3)
    ...
    for t in range(args.num_iters):
        ...
        if not args.patch_place == "fix_patch":
            ...
            # Z축의 그래디언트를 0으로 설정하여 X,Y축만 업데이트되도록 함
            group.grad[..., 2] = 0  # Z축(2)의 그래디언트를 0으로
