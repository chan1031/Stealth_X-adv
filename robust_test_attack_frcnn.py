...
def rotate_patch_180_torch(patch_tensor):
    """
    X-ray 패치를 180도 회전하는 함수 (PyTorch Tensor 사용).
    
    Args:
        patch_tensor (torch.Tensor): 회전할 패치 텐서 (1, H, W)
    
    Returns:
        torch.Tensor: 180도 회전된 패치
    """
    return TF.affine(patch_tensor, angle=180, translate=(0, 0), scale=1.0, shear=0)
    ...
    for pt in range(args.patch_count):
        ...
        for s in range(len(images_adv)):
            ...
            if len(bboxes[s]) > 0:
                ...
                if len(bboxes[s]) == 0:
                    continue  # 객체가 없는 경우 건너뜀

                if pt >= len(bboxes[s]):  # pt 값이 초과하면 마지막 인덱스로 보정
                    pt = len(bboxes[s]) - 1  # 마지막 객체 인덱스 사용
                    
                x_min, y_min, x_max, y_max = bboxes[s][pt]
                obj_center_x = (x_min + x_max) // 2
                obj_center_y = (y_min + y_max) // 2

                # 이미지 크기 가져오기
                image_height, image_width = images_adv[s].shape[1:]

                # 객체의 위치에 따라 패치 이동 방향 결정
                offset = 20 # 최소 거리
                if obj_center_x < image_width // 3:  # 객체가 왼쪽에 있음
                    move_x = offset  # 패치를 오른쪽으로 이동
                elif obj_center_x > 2 * image_width // 3:  # 객체가 오른쪽에 있음
                    move_x = -offset  # 패치를 왼쪽으로 이동
                else:
                    move_x = random.choice([-offset, offset])  # 중앙이면 랜덤

                if obj_center_y < image_height // 3:  # 객체가 위쪽에 있음
                    move_y = offset  # 패치를 아래쪽으로 이동
                elif obj_center_y > 2 * image_height // 3:  # 객체가 아래쪽에 있음
                    move_y = -offset  # 패치를 위쪽으로 이동
                else:
                    move_y = random.choice([-offset, offset])  # 중앙이면 랜덤

                # 최종 위치 조정
                new_u = max(0, min(u + move_y, image_height - args.patch_size))
                new_v = max(0, min(v + move_x, image_width - args.patch_size))
            else:
                # 객체가 없으면 이동 적용하지 않음
                new_u, new_v = u, v
            
            #회전추가
            rotated_patch = rotate_patch_180_torch(rend[0])
            
            # 패치 적용
            images_adv[s][:, new_u:new_u+args.patch_size, new_v:new_v+args.patch_size].mul_(rend[0])
